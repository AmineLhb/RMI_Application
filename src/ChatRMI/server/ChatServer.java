...
import ChatRMI.client.DB;
import ChatRMI.client.ChatClient3IF;
...
public class ChatServer extends UnicastRemoteObject implements ChatServerIF {
	String line = "---------------------------------------------\n";
	private Vector<ChatRMI.server.Chatter> chatters;
	private static final long serialVersionUID = 1L;
	
	//Constructor
	public ChatServer() throws RemoteException {
		super();
		chatters = new Vector<ChatRMI.server.Chatter>(10, 1);
	}
	...
	private void registerChatter(String[] details){		
		try{
			...
			chatters.addElement(new ChatRMI.server.Chatter(details[0], nextClient));
	...
	private void updateUserList() {
		...
		String[] currentUsers = q;

		for (ChatRMI.server.Chatter c : chatters) {
			try {
